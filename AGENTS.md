...
**Reuse:** Within one bounded task, reuse already-read canonical instructions and live-state snapshots. Re-read or rediscover only after a relevant mutation, failure, contradiction, external-change signal, or when freshness itself is material.
**GitHub org auth:** Treat `tbhrc` as the organisation, not a required CLI username; an authenticated principal with active `tbhrc` membership and the required target-repository permission is authorised, so do not re-authenticate or mint/switch credentials merely because `gh api user` returns a different username.
