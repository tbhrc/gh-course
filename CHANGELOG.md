...
- Temporary explicit-trigger GitHub Actions bridge for `AGENT_DISPATCH_TOKEN` and `PROJECT_MANAGEMENT_TOKEN`, syncing the canonical shared credential names to active non-fork repositories visible to the bridge credential while hard-failing when any explicit core operating repository is hidden; long-term organisation-level secret management is tracked in `tbhrc/ai-ops#7`.
